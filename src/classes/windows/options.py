...
class OPTIONS:
    def __init__(self, scr, start_window):
        ...
        self.font_jersey40 = LOADER.font.load(FONT_PATH, 50)
        self.font_jersey = LOADER.font.load(FONT_PATH, 55)

    def draw(self):
        pygame.Surface.fill(self.screen, 'black')

        return_button = self.font_jersey40.render(
        ...
        if return_button_rect.collidepoint(pos):
            ...
            return_button2 = self.font_jersey.render(
                TEXTS['return'], True, (229, 204, 255))
            self.screen.blit(
                return_button2,
                (return_button_x - 6,
